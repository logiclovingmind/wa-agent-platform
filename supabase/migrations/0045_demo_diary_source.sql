-- The demo diary, with a booking's origin written down rather than inferred.
--
-- Migration 0044 gave `appointments` a `source`, because a person booking from a thread
-- now sets `conversation_id` too and the diary could no longer read that column as "the
-- assistant did this". The backfill there fixed the rows that existed. This fixes the ones
-- that come back: the demo diary is rebuilt from scratch by a cron at 01:30 IST and by the
-- reset button, so without this every seeded booking returns as `person` and the demo org
-- opens on "None of them taken by the assistant" — the single sentence the whole demo is
-- there to disprove.
--
-- The body is restated rather than diffed, as in 0043 and for the same reason: 0040 is
-- applied and this is `create or replace`. The only change is `source` on the two inserts,
-- set from whether the row has a conversation — which is what it meant in this seed all
-- along, and is exactly what 0044 stopped being true in general.
...
begin
  ...
  select
    demo_org, c.id, s.starts_at, 30, v.customer_name, v.service,
    -- Only the row joined to a thread is the assistant's. The others are the walk-ins and
    -- phone calls somebody typed in, which is what the unlinked rows have always meant.
    case when c.id is null then 'person' else 'assistant' end
  from (
  ...
  insert into appointments
    (org_id, conversation_id, starts_at, duration_minutes, customer_name, service, status,
     source)
  select
    demo_org, c.id, s.starts_at, 30, v.customer_name, v.service, v.status,
    -- The no-show is the assistant's booking, the attended one was typed in by hand. The
    -- comment above already says so; this is the column that now carries it.
    case when c.id is null then 'person' else 'assistant' end
  from (
